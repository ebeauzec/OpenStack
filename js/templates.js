...
- **Ceph RBD Mirroring:** ${cephBackendUsed(cinderBackends, manilaBackends) && inputs.enableCinderReplication ? 'Enabled for Ceph-backed Cinder volumes -- the `rbd-mirror` daemon asynchronously replicates RBD images to a remote Ceph cluster over WAN interfaces.' : 'Not Configured. (Cinder Volume Replication is disabled below, or no Ceph Cinder backend is selected -- rbd-mirror is not active in this design.)'}
- **S3 Data Protection (StorageGrid):** ${enableStoragegrid ? 'StorageGrid is provisioned; S3 bucket replication rules are managed by its Information Lifecycle Management (ILM) engine, supporting dual-site active replica copies where configured.' : 'Not Configured. (StorageGrid object storage is not provisioned in this design.)'}
...
- **Kubernetes Velero Backup:** ${inputs.enableVelero ? (enableStoragegrid ? 'Enabled. Schedules backups of container namespaces and PV snapshots directly to the provisioned StorageGrid S3 target.' : 'Enabled, but no StorageGrid S3 target is provisioned in this design -- an S3-compatible bucket (external or third-party) must be supplied as the Velero BackupStorageLocation before this is deployable.') : 'Disabled.'}
...
export function generateBOM(inputs, computeResult, cephResult, networkResult) {
  ...
  // cinderProto/manilaProto are tracked SEPARATELY -- NetApp and VAST Data both support
  // Cinder (block) and Manila (file) with genuinely different protocols (e.g. NetApp's
  // Cinder protocol is whatever's configured in Step 3, but its Manila driver always uses
  // NFS/CIFS regardless of that setting; VAST is NVMe-oF/TCP for Cinder but NFS-only for
  // Manila). A single shared "proto" field previously showed the Cinder protocol even on
  // rows that were Manila-only, which is factually wrong for the Manila role.
  const externalStorageVendors = {
    netapp: { name: 'NetApp ONTAP', cinderProto: (inputs.netappProto || 'iscsi').toUpperCase(), manilaProto: 'NFS / CIFS', mgmtIp: inputs.netappIp },
    emc: { name: 'Dell EMC PowerFlex', cinderProto: 'SDC', manilaProto: null, mgmtIp: inputs.emcIp },
    pure: { name: 'Pure Storage FlashArray', cinderProto: (inputs.pureProto || 'iscsi').toUpperCase(), manilaProto: null, mgmtIp: inputs.pureIp },
    hpe: { name: `HPE ${inputs.hpePlatform === '3par' ? '3PAR' : 'Primera/Alletra'}`, cinderProto: (inputs.hpeProto || 'fc').toUpperCase(), manilaProto: null, mgmtIp: inputs.hpeIp },
    dellps: { name: `Dell ${inputs.dellpsPlatform === 'powermax' ? 'PowerMax' : 'PowerStore'}`, cinderProto: (inputs.dellpsProto || 'iscsi').toUpperCase(), manilaProto: null, mgmtIp: inputs.dellpsIp },
    vast: { name: 'VAST Data', cinderProto: 'NVMe-oF/TCP', manilaProto: 'NFS', mgmtIp: inputs.vastIp }
  };
  ...
  const externalStorageRows = activeExternalVendors.map(b => {
    ...
    const protoParts = [
      usedForCinder ? `${v.cinderProto} (Cinder)` : null,
      usedForManila && v.manilaProto ? `${v.manilaProto} (Manila)` : null
    ].filter(Boolean);
    // If only one role is active, drop the "(Cinder)"/"(Manila)" suffix -- it's redundant
    // with the Role column in that case.
    const protoDisplay = protoParts.length === 1 ? protoParts[0].replace(/ \((Cinder|Manila)\)$/, '') : protoParts.join(' / ');
    return `| ${v.name} | ${role} | ${protoDisplay} | Management target: \`${v.mgmtIp || 'n/a'}\` | 1 array/cluster (capacity per model is vendor-specific -- validate against the vendor's own sizing tool) |`;
  }).join('\n');
...
- **Protocol:** \`${(inputs.netappProto || 'iscsi').toUpperCase()}\` (runs over Dedicated Storage network).
...
export function generateK8sVelero(inputs) {
  ...
  const s3Url = enableStoragegrid ? `https://${storagegridIp}:${storagegridPort}` : 'https://REPLACE-WITH-YOUR-S3-ENDPOINT';
  const bslName = enableStoragegrid ? 'storagegrid-s3' : 's3-backup-target';

  return `# Kubernetes Velero Backup Configuration & Manifests
# File: velero-backup.yaml
# IMPLICATIONS: Backup of Kubernetes namespaces and CSI persistent volumes to an S3-compatible target.
# Incremental backups are executed via Restic/Kopia.
${enableStoragegrid ? '# Target: NetApp StorageGrid S3 (provisioned in this design).' : `# NOTE: This design does not provision StorageGrid or any other S3-compatible object store.
# Velero requires one -- substitute your own S3-compatible endpoint/bucket/credentials below
# before applying this manifest. The values here are placeholders only.`}
...
export function generateProposalDesign(inputs, computeResult, cephResult) {
  ...
  // Cinder attach-protocol summary (Section 3.1) -- built from the actually-selected
  // Cinder backends, not a single vendor's protocol field standing in for all of them.
  const cinderAttachProtoMap = {
    ceph: 'librbd (mapped directly inside the QEMU process, no host-level block device)',
    netapp: netappProto.toUpperCase(),
    emc: 'the PowerFlex Storage Data Client (SDC) local block device',
    pure: pureProto === 'fc' ? 'Fibre Channel' : pureProto === 'iscsi' ? 'iSCSI' : 'NVMe-oF',
    hpe: hpeProto === 'fc' ? 'Fibre Channel' : 'iSCSI',
    dellps: dellpsProto === 'nvme-tcp' ? 'NVMe-TCP' : dellpsProto === 'fc' ? 'Fibre Channel' : 'iSCSI',
    vast: 'NVMe-oF/TCP'
  };
  const cinderAttachProtoSummary = Array.from(new Set(cinderBackends))
    .map(b => cinderAttachProtoMap[b]).filter(Boolean).join(' or ') || 'the backend-specific attach protocol';

  // Manila share-lifecycle summary (Section 3.2) -- built from the actually-selected
  // Manila backends, not a hardcoded CephFS/NetApp pairing regardless of selection.
  const manilaLifecycleMap = {
    cephfs_native: 'compute nodes mount CephFS shares directly via the native kernel/ceph-fuse client',
    cephfs_ganesha: 'shares are re-exported over NFS through an NFS-Ganesha gateway in front of CephFS',
    netapp: 'NetApp ONTAP configures export policies and routes traffic through dedicated Storage Virtual Machine interfaces',
    vast: 'VAST Data presents NFS shares directly from the VMS-managed cluster'
  };
  const manilaLifecycleSummary = Array.from(new Set(manilaBackends))
    .map(b => manilaLifecycleMap[b]).filter(Boolean).join('; ') || 'the configured Manila backend\'s native export mechanism';
...
${neutronDriver === 'ovn' ? `### 2.1 Neutron OVN SDN Advantage
...
* **Operational Simplicity:** Legacy agents (\`neutron-l3-agent\`, \`neutron-dhcp-agent\`) are replaced by standard OVS interfaces, lowering troubleshooting complexity.` : `### 2.1 Neutron ML2/OVS SDN Architecture
This deployment uses the legacy **ML2/OVS** driver for Neutron:
* **Agent-Based Services:** Layer 3 routing and DHCP are handled by dedicated \`neutron-l3-agent\` and \`neutron-dhcp-agent\` processes, typically concentrated on Network/Controller nodes rather than distributed to every compute host.
* **Flow Programming:** Open vSwitch flows are programmed directly by the ML2 plugin and its L2 agent on each hypervisor, without an intermediate OVN logical-flow database.
* **Operational Note:** ML2/OVS remains widely deployed and well understood, but lacks OVN's distributed L3/DHCP model -- factor Network node sizing and HA accordingly.`}
...
* **Hypervisor Direct Mount:** Cinder manages block device lifecycles (creation, deletion, snapping), but the active read/write data paths flow directly from Compute nodes to the storage array:
  * **QEMU Integration:** Libvirt configures the VM to attach the volume directly via **${cinderAttachProtoSummary}** over the storage network, bypassing any intermediate OS mounts on the hypervisor host.
...
* **Multi-Tenant Share Exports:** Manila provisions shared filesystems dynamically. For this design's selected backend(s): ${manilaLifecycleSummary}.
...
export function generateLiveTopologySVG(inputs, computeResult, cephResult) {
  ...
  <g transform="translate(115, 85)">
    <rect x="0" y="0" width="75" height="28" rx="5" ry="5" fill="rgba(240, 31, 38, 0.08)" stroke="#f01f26" stroke-width="1" />
    <text x="37" y="17" font-family="'Outfit', sans-serif" font-size="9px" font-weight="600" fill="#f3f4f6" text-anchor="middle">${neutronDriver === 'ovn' ? 'OVN DBs' : 'Neutron ML2/OVS'}</text>
  </g>
`;
