...
function setupEventListeners() {
  ...
  elements.stepperItems.forEach(item => {
    item.addEventListener('click', () => {
      ...
      const movingForwardPastManilaGate = targetStep > state.currentStep && targetStep >= 4;
      if (targetStep < state.currentStep || (validateStep(state.currentStep) && (!movingForwardPastManilaGate || validateManilaCompat()))) {
        goToStep(targetStep);
      } else if (movingForwardPastManilaGate) {
        goToStep(4);
        showToast('Resolve the Manila DHSS/backend compatibility warning below before continuing.');
      }
    });
  });
...
function goToStep(stepNumber) {
  ...
  // The Manila DHSS/backend compat gate only applies (and is only visible) on step 4 --
  // resync it here so Next is never left stuck disabled on an unrelated step (e.g. after
  // navigating away from step 4 without fixing an incompatibility).
  if (stepNumber === 4) {
    validateManilaCompat();
  } else {
    const manilaWarning = document.getElementById('manila-compat-warning');
    if (manilaWarning) manilaWarning.style.display = 'none';
    elements.btnNext.disabled = false;
  }
